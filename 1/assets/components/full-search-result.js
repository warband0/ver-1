...
import { tableRow } from "./full-table-row.js"
...
export class SearchResult {
  ...
  constructor(catalog, parentElement) {
    ...
    this.outerTitle = newElement('p', ['search-results__title'], 'Наружн. Ø mm');
    this.screenTitle = newElement('p', ['search-results__title'], 'Экран');
    this.VoltageTitle = newElement('p', ['search-results__title'], 'Напряжение, В');
    this.conditionTitle = newElement('p', ['search-results__title'], 'Условия эксплуатации; температура/подвижно');
    this.PeculiaritiesTitle = newElement('p', ['search-results__title'], 'Особенности');
    this.pairsTitle = newElement('p', ['search-results__title'], 'Кол-во пар, жил, троек');
    this.cutTitle = newElement('p', ['search-results__title'], 'Сечение или диаметр, мм/мм2/мкм/AWG');
    this.totalPairsTitle = newElement('p', ['search-results__title'], 'Общее кол-во жил');
    this.insulationTitle = newElement('p', ['search-results__title'], 'Материал изоляции');
    this.shellTitle = newElement('p', ['search-results__title'], 'Материал оболочки');
    this.temperatureTitle = newElement('p', ['search-results__title'], 'Температура/стационарно');
    this.colorTitle = newElement('p', ['search-results__title'], 'Цвет');
    ...
    this.titlesContainer.append(this.articleTitle, this.typeTitle, this.markTitle, this.sizeTitle,this.outerTitle, this.screenTitle, this.VoltageTitle, this.conditionTitle, this.priceTitle, this.PeculiaritiesTitle, this.pairsTitle, this.cutTitle, this.totalPairsTitle, this.insulationTitle, this.shellTitle, this.temperatureTitle, this.colorTitle, this.AllAmountTitle, this.cablesTitle);
