import { tableRow } from "../../assets/components/table-row.js"
...
console.log(data);
const fastSearchCriterions = [
  "Тип",
  "Марка",
  "Маркоразмер",
  "Экран",
  "Условия эксплуатации; температура/подвижно"
]
...
const inputButton = document.querySelector('.article-button')
inputButton.addEventListener('click', () => {
  
  const oldResult = document.querySelector('.search-results')
  if(oldResult) {
  ...
  const result = data.find( value => {
    if(value["Артикул"] === Number(input.value)) {
      return value
    }
    
  })
  const searhRes = new SearchResult([result], main, 0)
})