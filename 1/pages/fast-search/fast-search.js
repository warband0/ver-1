...
import { SearchResult } from "../../assets/components/search-result.js";
...
class Filter {
  ...
  constructor(data, SearchResultsContainer, parentElement, allCriterions, fastSearchCriterions) {
    ...
    this.searchButton = newElement('button', ['search-button'], 'Найти')
  ...
  search() {
    ...
    for (let conditions in this.searchConditions) {
      ...
      if (this.searchEmpty) {
        ...
        const container = document.querySelector('.search-results__results-container')
        
        container.append(tryAnotherCriterions)
        this.fieldSets.forEach (value => {
